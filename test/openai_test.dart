...
void main() {
  ...
  setUpAll(() async {
    // 读取环境变量
    apiKey =
        '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';
    ...
    await DatabaseManager.getInstance(
      'test',
    ).initialize(storagePath: 'D:\\project\\GitHub\\wenzagent\\test_db');
  ...
  group('基础功能测试', () {
    test('✅ 发送消息并收到回复', () async {
      ...
      final messageId = await cachedProxy.sendMessage(
        MessageInput(content: '你好，请简单回复"测试成功"'),
      );
    ...
    test('✅ 消息ID一致性', () async {
      ...
      // 发送消息
      final returnedId = await cachedProxy.sendMessage(
        MessageInput(id: clientMessageId, content: '测试ID一致性'),
      );
    ...
    test('✅ 消息不重复', () async {
      ...
      // 发送一条消息
      await cachedProxy.sendMessage(MessageInput(content: '测试消息不重复'));
    ...
    test('✅ 状态监听', () async {
      ...
      // 发送消息
      await cachedProxy.sendMessage(MessageInput(content: '测试状态监听'));
      ...
      final hasProcessing = states.any(
        (s) =>
    ...
    test('✅ 清空消息', () async {
      ...
      // 发送多条消息
      for (int i = 0; i < 2; i++) {
        await cachedProxy.sendMessage(MessageInput(content: '测试消息 $i'));
        await _waitForIdle(cachedProxy, timeout: const Duration(seconds: 30));
      }
