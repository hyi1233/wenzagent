...
class TaskComplexityTool extends AgentTool {
  ...
  String _buildAnalysisPrompt(String task, String? context) {
    ...
    buffer.writeln('你是一个开发任务复杂度评估专家。请根据以下任务描述，判断其复杂度等级并给出委派建议。');
    buffer.writeln();
    buffer.writeln('重要：主 Agent 是纯粹的规划者和委派者，不直接执行任何文件操作或命令。所有实际工作都通过 spawn_sub_agent 委派给子 Agent 执行。复杂度等级仅影响规划和委派策略。');
    buffer.writeln();
    ...
    buffer.writeln();
    buffer.writeln('**复杂度等级**：简单/中型/复杂');
    buffer.writeln();
    buffer.writeln('**判断依据**：(简要说明为什么是这个等级)');
    buffer.writeln();
    buffer.writeln('**委派策略**：(根据对应等级的做法，给出具体的委派步骤建议，包括如何拆分任务和描述子任务)');
    buffer.writeln();
